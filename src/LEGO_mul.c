...
int main (int argc, char *argv[]) {	
	...
	boolean **intG;
	int intNum,*intGNum,*intGNum_net;
	int i,nodeNum,gsNum,intSize;
	int geneset_min, geneset_max;
// argc	
	if (argc < 7) {
		fprintf(stderr, "Usage: %s <gene set file> <node weight file> <contribution score file> <gene set info file> <interesting gene list file> <minimun gene set size> <maximum gene set size>\n",argv[0]); 
		exit(0); 
	} 
// argc --> par	
...
// read in interesting gene list and calculate ES
	...
	intGNum = (int *)malloc((MAX_IG_SIZE+1)*sizeof(int)); 
	intGNum_net = (int *)malloc((MAX_IG_SIZE+1)*sizeof(int)); 
	ReadIG_mul_file(input_int_file,input_GS_file,&intG,&intGNum,&intNum,&intGNum_net);
	LEGOout_int_mul_file(intG,intGNum,intGNum_net,intNum,gsNum,geneset_min,geneset_max,output_ES_file,input_NW_file,input_CS_file,input_GS_file);
