...
foreach $each (@num){
	$final_file = "$mid_data_dir/$bg\_$each.RData";
	if(-e $final_file){
		next;
	}
	$tmp_input = $geneset."_$bg\_input_$each";
	...
	print "perl $main_dir/LEGO_noperm.pl $net_file $geneset_file $tmp_input -multi 1 -noR 0 \n";
	system "perl $main_dir/LEGO_noperm.pl $net_file $geneset_file $tmp_input -multi 1 -noR 0 ";
